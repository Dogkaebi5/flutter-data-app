...
class UserInterestData extends ChangeNotifier {
  ...
  List _loan = List.filled(Questions().interests["대출"]!.length, _tempString);
  List _deposit = List.filled(Questions().interests["예금/적금"]!.length, _tempString);
  List _immovables = List.filled(Questions().interests["부동산"]!.length, _tempString);
  List _stock = List.filled(Questions().interests["주식"]!.length, _tempString);
  List _cryto = List.filled(Questions().interests["가상자산"]!.length, _tempString);
  List _golf = List.filled(Questions().interests["골프"]!.length, _tempString);
  List _tennis = List.filled(Questions().interests["테니스"]!.length, _tempString);
  List _fitness = List.filled(Questions().interests["피트니스"]!.length, _tempString);
  List _yoga = List.filled(Questions().interests["요가"]!.length, _tempString);
  List _dietary = List.filled(Questions().interests["건강식품"]!.length, _tempString);
  List _educate = List.filled(Questions().interests["교육"]!.length, _tempString);
  List _parental = List.filled(Questions().interests["육아용품"]!.length, _tempString);
  List _automobile = List.filled(Questions().interests["자동차"]!.length, _tempString);
  List _localTrip = List.filled(Questions().interests["국내여행"]!.length, _tempString);
  List _overseatrip = List.filled(Questions().interests["해외여행"]!.length, _tempString);
  List _camp = List.filled(Questions().interests["캠핑"]!.length, _tempString);
  List _fishing = List.filled(Questions().interests["낚시"]!.length, _tempString);
  List _pet = List.filled(Questions().interests["반려동물"]!.length, _tempString);
