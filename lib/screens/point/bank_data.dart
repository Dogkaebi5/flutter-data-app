  ...
  Future<String?> bankListDialog() async{
    ...
    await showDialog(context: context,
      builder:(context){return StatefulBuilder(builder:(BuildContext context, StateSetter setState){
        return Dialog.fullscreen(backgroundColor: const Color.fromARGB(255, 240, 240, 240),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                IconButton(icon: const Icon(Icons.close),iconSize: 30,
                  onPressed: () => Navigator.pop(context)),
                const Text('은행선택', style: fontSmallTitle),
                const SizedBox(width: 48, height: 60)]),
                SingleChildScrollView(child: SizedBox(width: double.infinity,
                    ...
                    children: [
                      for(var i = 0; i < bankList.length; i++)
                        InkWell(
                          onTap: (){
                            setState(() {
                              for(int j = 0; j < isSelectBanks.length; j++){isSelectBanks[j] = false;}
                              isSelectBanks[i] = !isSelectBanks[i];
                              newBank = bankList[i];});
                            Navigator.pop(context);
                          },
                          child: Container(width: MediaQuery.of(context).size.width/3 - 20,
                            margin: const EdgeInsets.all(4), padding: const EdgeInsets.all(14),
